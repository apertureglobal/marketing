...
SRC = ("/private/tmp/claude-501/-Users-kyleforeman-Documents-GitHub-APERTUREREPO-"
       "Untitled/89605ee2-e0e2-42b1-a533-95bf6685f64a/scratchpad/wphotos")
OUT = ("/Users/kyleforeman/Documents/GitHub/APERTUREREPO/Untitled/documents/"
       "Alex Brandau IV/September 2026/Strategy Documents/assets/ads-806willowdale")
DPI = 200
...
# Cover: the facade under its trees. Type sits low, so the scrim deepens downward.
...
finish(arr, f"{OUT}/willowdale-cover-flat.jpg")

# Back cover: the pool and grounds. Headline and agent block are centred, so it
# deepens mid-page.
arr = np.asarray(ImageEnhance.Color(load("48.jpg")).enhance(0.82), dtype=np.float64)
arr = arr * 0.94
prof = 0.34 + 0.46 * np.exp(-((rows - 0.52) ** 2) / (2 * 0.26 ** 2))
prof = np.clip(prof + 0.16 * np.clip((rows - 0.80) / 0.20, 0, 1), 0, 0.93)
arr = inset_rule(hatch(scrim(arr, prof)))
finish(arr, f"{OUT}/willowdale-back-flat.jpg")
