...
Preview page: **[preview.html](preview.html)** — all six units on one page with play/pause,
replay and a 15 s scrubber.

## Units

**This campaign ships these six** — the established Aperture set, one layout each:
...
**Built but not shipped on this campaign.** These four are kept on disk as the reference
standard for refining those sizes separately; they are deliberately absent from
`preview.html`, which shows the six advertisements the strategy document describes. Each is
adapted from the closest-proportion layout above rather than newly invented:
...
Generated, never hand-edited. Editing a unit's `index.html` is reverted by the next rebuild.
    ...
    tools/aperture/build_lakeview.sh                    # all ten built sizes
...
1. **Sizes.** This campaign ships the standard Aperture six — 1024×768, 970×250, 768×1024,
   480×320, 320×480, 300×600 — which is the set the template's six layouts are built for and
   the set Aperture display campaigns run. 300×250, 728×90, 160×600 and 320×50 were built as
   reference but are held back from this flight.
...
5. **JPEG quality is the only budget lever here**, since there is no video bitrate to trade
   ...
   the shipped six sit at q56–q62. Adding a seventh still, or raising 1024×768 above q40,
   would push it over. The four held-back sizes have small bands, so they run at q72–q78 and
   land at 6–16% of budget — no budget pressure anywhere in that group.
...
8. **`bg.jpg` for the four held-back sizes** is cover-cropped from the nearest-aspect shipped
...
Real browser, served over HTTP: all six shipped units report no broken images, animations running,
